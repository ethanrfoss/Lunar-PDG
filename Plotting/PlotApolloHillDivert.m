function PlotApolloHillDivert(System,x,u)
...
set(gca,'color',[0 0 0])
[X,Y]=meshgrid(linspace(1.1*min(min(x(1,:))),1.1*max(max(x(1,:))),20),linspace(1.1*min(min(x(2,:))),1.1*max(max(x(2,:))),20));
surf(X,Y,0.*X);
Z = -(X.^2+Y.^2)/System.HillWidth + System.HillHeight;
surf(X,Y,Z);
colormap('gray');

axis([1.1*min(min(x(1,:))) 1.1*max(max(x(1,:))) 1.1*min(min(x(2,:))) 1.1*max(max(x(2,:))) 0 1.1*max(max(x(3,:)))])
