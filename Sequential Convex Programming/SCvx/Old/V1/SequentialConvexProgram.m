...
function [L,x,u,p,vars] = SequentialConvexProgram(SCP,System,xbar,ubar,pbar,Cost,Dynamics,ConvexConstraints,NonConvexConstraints,InitialCondition,FinalCondition,Scaling,Continuation)
...
[N,nx,nu,np,nv,ns,nic,ntc,nlx,nlu,nlp,nqx,nqu,nqp,nbx,nbu] = Dimensions(System,xbar,ubar,pbar,Cost,Dynamics,ConvexConstraints,NonConvexConstraints,InitialCondition,FinalCondition);
...
% Scaling Matrices:
[Sx,Su,Sp,cx,cu,cp] = ScalingMatrices(System,Scaling);

% Discretize:
[A,Bm,Bp,F,r,E,delta] = DiscretizeDynamics(SCP,System,xbar,ubar,pbar,t,nx,nu,np,nv,N,Dynamics);
...
[s,C,D,G,rp] = DiscretizeNonConvexConstraints(System,xbar,ubar,pbar,t,nx,nu,np,ns,N,NonConvexConstraints);
...
Jbar = NonLinearCost(SCP,xbar,ubar,pbar,delta,N,lx,lu,lp,Bx,Bu,s,gic,gtc);
...
while true
    ...
    if strcmp(SCP.Solver,'ECOS')
        [cs,As,bs,Gs,hs,Ls,Qs] = Parse(SCP,xbar,ubar,pbar,lx,lu,lp,Bx,Bu,A,Bm,Bp,F,r,E,WLx,yLx,WLu,yLu,WLp,yLp,WQx,yQx,WQu,yQu,WQp,yQp,dqx,dqu,dqp,C,D,G,rp,H0,K0,l0,Hf,Kf,lf,Sx,Su,Sp,cx,cu,cp,N,nx,nu,np,nv,ns,nic,ntc,nlx,nlu,nlp,nqx,nqu,nqp,nbx,nbu);
...
function [N,nx,nu,np,nv,ns,nic,ntc,nlx,nlu,nlp,nqx,nqu,nqp,nbx,nbu] = Dimensions(System,xbar,ubar,pbar,Cost,Dynamics,ConvexConstraints,NonConvexConstraints,InitialCondition,FinalCondition)

[lx,lu,lp,Bx,Bu] = Cost();
[f,A,B,F,E] = Dynamics(System,xbar(:,1),ubar(:,1),pbar,0);
[WLx,yLx,WLu,yLu,WLp,yLp,WQx,yQx,WQu,yQu,WQp,yQp,dqx,dqu,dqp] = ConvexConstraints(System,xbar(:,1),ubar(:,1),pbar,0);
[s,C,D,G] = NonConvexConstraints(System,xbar(:,1),ubar(:,1),pbar,0);
...
% Running Cost:
Gamma = (vecnorm(Bx*xbar) + vecnorm(Bu*ubar) + SCP.lambda*vecnorm(delta) + SCP.lambdas*vecnorm(max(s,0))) * [1; ones(N-2,1);1]*1/N;
...
function [c,A,b,G,h,L,Q] = Parse(SCP,xbar,ubar,pbar,lx,lu,lp,Bx,Bu,A,Bm,Bp,F,r,E,WLx,yLx,WLu,yLu,WLp,yLp,WQx,yQx,WQu,yQu,WQp,yQp,dqx,dqu,dqp,C,D,G,rp,H0,K0,l0,Hf,Kf,lf,Sx,Su,Sp,cx,cu,cp,N,nx,nu,np,nv,ns,nic,ntc,nlx,nlu,nlp,nqx,nqu,nqp,nbx,nbu)
...
for i = 1:N-1
    ...
    rv(nx*(i-1)+1:nx*i) = cx - r(1:nx,i) - A(1:nx,1:nx,i)*cx - Bm(1:nx,1:nu,i)*cu - Bp(1:nx,1:nu,i)*cu - F(1:nx,1:np,i)*cp;
...
A = [AM BM FM EM zeros((N-1)*nx,N*ns) zeros((N-1)*nx,nic) zeros((N-1)*nx,ntc) zeros((N-1)*nx,N) zeros((N-1)*nx,N) zeros((N-1)*nx,N) zeros((N-1)*nx,N) zeros((N-1)*nx,N-1) zeros((N-1)*nx,N) zeros((N-1)*nx,1) zeros((N-1)*nx,1); % Dynamics Equality Constraint
     H0M zeros(nic,N*nu) K0*Sp zeros(nic,(N-1)*nv) zeros(nic,N*ns) eye(nic,nic) zeros(nic,ntc) zeros(nic,N) zeros(nic,N) zeros(nic,N) zeros(nic,N) zeros(nic,N-1) zeros(nic,N) zeros(nic,1) zeros(nic,1); % Initial Condition Equality Constraint
     HfM zeros(ntc,N*nu) Kf*Sp zeros(ntc,(N-1)*nv) zeros(ntc,N*ns) zeros(ntc,nic) eye(ntc,ntc) zeros(ntc,N) zeros(ntc,N) zeros(ntc,N) zeros(ntc,N) zeros(ntc,N-1) zeros(ntc,N) zeros(ntc,1) zeros(ntc,1)]; % Final Condition Equality Constraint
...
for i = 1:N
    ...
    rpv(ns*(i-1)+1:ns*i) = -rp(1:ns,i) - C(1:ns,1:nx,i)*cx - D(1:ns,1:nu,i)*cu - G(1:ns,1:np,i)*cp;
...
function [A,Bm,Bp,F,r,E,delta] = DiscretizeDynamics(SCP,System,xbar,ubar,pbar,t,nx,nu,np,nv,N,Dynamics)
...
for k = 1:N-1
    ...
    P0 = Flatten(xbar(:,k),phi0,PBm0,PBp0,PF0,Pr0,nx,nu,np);
    
    % Integrate from current time step to next:
    tspan = linspace(t(k),t(k+1),SCP.Nsub);
    P = RK4(@(tc,x) Derivatives(System,x,tc,ubar(:,k+1),ubar(:,k),pbar,t(k+1),t(k),nx,nu,np,Dynamics),tspan,P0);
...
function [x,Pphi,PBm,PBp,PF,Pr] = Unflatten(P,nx,nu,np)
...
Pr = P(nx+nx^2+2*nx*nu+nx*np+1:2*nx+nx^2+2*nx*nu+nx*np);

end

function P = Flatten(x,Phi,PBm,PBp,PF,Pr,nx,nu,np)
...
% Compute Dynamics
[f,A,B,F,~] = Dynamics(System,x,u,p,t);
...
% Vectorize Result
Pdot = Flatten(Pxdot,Pphidot,PBmdot,PBpdot,PSdot,PRdot,nx,nu,np);

end

function [WLx,yLx,WLu,yLu,WLp,yLp,WQx,yQx,WQu,yQu,WQp,yQp,dqx,dqu,dqp] = DiscretizeConvexConstraint(System,xbar,ubar,pbar,t,nx,nu,nlx,nlu,nqx,nqu,N,ConvexConstraints)
...
for i = 1:N
    ...
    [WLx(1:nlx,1:nx,i),yLx(1:nlx,i),WLu(1:nlu,1:nu,i),yLu(1:nlu,i),WLp,yLp,WQx(1:nqx,1:nx,i),yQx(1:nqx,i),WQu(1:nqu,1:nu,i),yQu(1:nqu,i),WQp,yQp,dqx,dqu,dqp] = ConvexConstraints(System,xbar(:,i),ubar(:,i),pbar,t(i));

end

end

function [s,C,D,G,rp] = DiscretizeNonConvexConstraints(System,xbar,ubar,pbar,t,nx,nu,np,ns,N,NonConvexConstraints)
...
for i = 1:N
    ...
    [s(1:ns,i),C(1:ns,1:nx,i),D(1:ns,1:nu,i),G(1:ns,1:np,i)] = NonConvexConstraints(System,xbar(:,i),ubar(:,i),pbar,t(i));
    rp(1:ns,i) = s(1:ns,i) - C(1:ns,1:nx,i)*xbar(:,i) - D(1:ns,1:nu,i)*ubar(:,i) - G(1:ns,1:np,i)*pbar;

end

end

function [Sx,Su,Sp,cx,cu,cp] = ScalingMatrices(System,Scaling)
